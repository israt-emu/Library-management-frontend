...
import {useAddBookmarkMutation, useGetGroupsQuery} from "../../features/bookmark/bookmarkAPI";
import {useSelector} from "react-redux";
import Error from "../ui/Error";
import BookmarkFlatCard from "./BookmarkFlatCard";
...
const BookMarks = () => {
  ...
  //       {/* // */}
  //       <div className="grid grid-cols-1 md:grid-cols-3 justify-center items-center gap-4 mt-8 pb-8">{bookmarksByState?.map((b) => (b?.group === group ? <BookmarkCard key={b?._id} bookmark={b} currentState="normal" setShowModal={setShowModal} showModal={showModal} /> : null))}</div>
  //     </div>
  //   ));
  // }
