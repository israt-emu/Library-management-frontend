...
const Trash = () => {
  ...
  // const {data: groupData} = useGetGroupsQuery({state:"trash",userId:id});
  // //filtering by state
  // useEffect(() => {
  //   if (!bookmarkData?.isError && !bookmarkData?.isLoading && bookmarkData?.bookmarks?.length > 0) {
  //     const filter = bookmarkData?.bookmarks?.filter((b) => b?.state === "trash");
  ...
  //       {/* // */}
  //       <div className="grid grid-cols-1 md:grid-cols-3 justify-center items-center gap-4 mt-8 pb-8">{bookmarksByState?.map((b) => (b?.group === group ? <BookmarkCard key={b?._id} bookmark={b} currentState="trash" /> : null))}</div>
  //     </div>
  //   ));
  // }
  ...
  //       <h1 className="border-b-2 pb-2 w-1/6">{group.toUpperCase()}</h1>
  //       {/* // */}
  //       <div className="grid grid-cols-1 justify-center items-center gap-3 mt-8 pb-8">{bookmarksByState?.map((b) => (b?.group === group ? <BookmarkFlatCard key={b?._id} bookmark={b} currentState="trash" /> : null))}</div>
  //     </div>
  //   ));
  // }
  ...
  //   content = (
  //     <div className="grid grid-cols-1 md:grid-cols-3 justify-center items-center gap-4 mt-12 ">
  //       <h1>No bookmark Found</h1>
  //     </div>
  //   );
  // }
  return <section className="overflow-y-scroll no-scrollbar h-full">Trash</section>;
};
